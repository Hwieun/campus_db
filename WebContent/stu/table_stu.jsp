...
sql = "SELECT course_name,time,view_count FROM db_hw.`course material` WHERE subject_name =" +session.getAttribute("subject_name"+ flag);
...
%>
	...
	<div id="grid3">
		<div id="left">
			<br> <br> <br> <br> <br>
			<p class="one">
				<a href="../stu/table_stu.jsp?flag=1" style="font-weight: bold">▶ 강의 자료실</a>
			</p>
			<br> <br> <br>
			</p>
		</div>
		<div id="right">
			...
			<form name = "form" action="myservlet.do" method="POST"
				...
				<select name="subject" id="myselect" onchange="subjectMenu(this.form)">
					...
					<option  value="../stu/table_stu.jsp?flag=1"><%=session.getAttribute("subject_name1")%></option>
					<%if(session.getAttribute("subject_name2")!=null){ %>
					<option value="../stu/table_stu.jsp?flag=2"><%=session.getAttribute("subject_name2")%></option><%} %>
					<%if(session.getAttribute("subject_name3")!=null){ %>
					<option value="과목3"><%=session.getAttribute("subject_name3")%></option><%} %>
				</select>
			</form>
			<table style="margin-top: 10px;">
				...
				<tr>
					<td><%= j++ %></td>
					<td><a href="../showresource/showresource_stu.jsp?title=<%=next_flag%>&num=<%=j - 1%>&flag=<%=flag%>"><%= queryResult.getString(1) %></a></td>
