...
<body>
	<%
		...
		sql = "SELECT course_name,time,view_count FROM db_hw.`course material` WHERE subject_name = "
				+ session.getAttribute("subject_name1");

		System.out.println(sql);
		System.out.println("talbe_prof page, flag = " + flag);
		queryResult = statement.executeQuery(sql);

		System.out.println("");
		System.out.println("/--table_prof--/");

		System.out.println("resetting parameters...");
		session.setAttribute("prof_id", session.getAttribute("prof_id"));
		session.setAttribute("prof_name", session.getAttribute("prof_name"));
		session.setAttribute("job", session.getAttribute("job"));
		session.setAttribute("subject_name1", session.getAttribute("subject_name1"));
	...
	<div id="grid3">
		<div id="left">
			<br> <br> <br>
			<p class="one">
				<a href="../Prof/table_prof.jsp?flag=1" style="font-weight: bold">▶
					강의 자료실</a>
			</p>
			<br> <br> <br>
			<p class="one">
				<a href="../Prof/student_info.jsp">▶ 수강생 조회 </a>
			</p>
		</div>
		<div id="right">
			<h2 style="margin-left: 20px;">강의 자료실</h2>
			<form name="form" action="myservlet.do" method="POST"
				style="margin-left: 30px; width: 100px;">
				<select name="subject" id="myselect"
					onchange="subjectMenu(this.form)">
					<option value="subject">--선택--</option>
					<option value="../Prof/table_prof.jsp?flag=1"><%=session.getAttribute("subject_name" + flag)%></option>
				</select>
			</form>
			<table style="margin-top: 10px;">
				...
				<%
					...
					while (queryResult.next()) {
						String next_flag = queryResult.getString(1);
				%>
				<tr>
					<td><%=j++%></td>
					<td><a
						href="../showresource/showresource_prof.jsp?title=<%=next_flag%>&num=<%=j - 1%>&flag=<%=flag%>"><%=queryResult.getString(1)%></a></td>
